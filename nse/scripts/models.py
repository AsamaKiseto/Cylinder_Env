...
class RBCModel_FNO2(RBCModel):
    ...
    def pred_loss(self, ipt, ctr, opt):
        ...
        loss3 = ((ctr_rec-ctr) ** 2).mean()
        loss4 = rel_error(trans_out, out_latent).mean()
        loss6 = ((Lpde_rbc(ipt, out_pred, self.dt, Re = self.Re, Lx = self.Lx, Ly = self.Ly) + mod_pred) ** 2).mean()
...
class RBCModel_FNO_test(RBCModel):
    ...
    def pred_loss(self, ipt, ctr, opt):
        ...
        loss3 = rel_error(ctr_rec, ctr).mean()
        loss4 = rel_error(trans_out, out_latent).mean()
        loss6 = ((Lpde(ipt, out_pred, self.dt, Re = self.Re, Lx = self.Lx, Ly = self.Ly)) ** 2).mean()
