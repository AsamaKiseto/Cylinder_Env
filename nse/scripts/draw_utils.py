...
def test_plot(t_nn, log_list, scale_k, ex_name = 'fb_0.0', fig_name = 'test'):
    ...
    ax[fig_num - 1].set_xlabel("t", fontsize=10)
    ...
    for k in range(len(log_list)):
        ...
        for j in range(len(scale_k)):
            ...
            # ax[2].fill_between(t_nn, Lpde_obs_v[0][scale_k[j]], Lpde_obs_v[1][scale_k[j]], alpha=0.2)
            # ax[3].fill_between(t_nn, Lpde_pred_v[0][scale_k[j]], Lpde_pred_v[1][scale_k[j]], alpha=0.2)
            
            ax[0].legend()
            
    plt.savefig(f'logs/pics/phase1_{fig_name}_{ex_name}.jpg')
    ...
    ax[0].set_title("Error/Loss in Different Scales", fontsize=15)
    ax[0].set_ylabel(r'Cul $C_D$ loss', fontsize=10)
    ax[1].set_ylabel(r'Cul $C_L$ loss', fontsize=10)
    ax[fig_num-1].set_xlabel("t", fontsize=10)
    
    for k in range(len(log_list)):
        data_list = torch.load(f'logs/data/phase1_test_{log_list[k]}_{ex_name}')
        _, _, _, _, _, _, _, error_Cd_cul, error_Cl_cul = calMean(data_list)
        _, _, _, _, _, _, _, error_Cd_cul_v, error_Cl_cul_v = calVar(data_list)
    ...
    plt.savefig(f'logs/pics/phase1_coef_cul_{fig_name}_{ex_name}.jpg')
    ...
    for k in range(len(log_list)):
        data_list = torch.load(f'logs/data/phase1_test_{log_list[k]}_{ex_name}')
        _, _, _, _, _, error_Cd_1step, error_Cl_1step, _, _ = calMean(data_list)
        _, _, _, _, _, error_Cd_1step_v, error_Cl_1step_v, _, _ = calVar(data_list)
    ...
    ax[0].set_title("Error/Loss in Different Scales", fontsize=15)
    ax[0].set_ylabel("PDE loss of obs", fontsize=10)
    ax[1].set_ylabel("PDE loss of pred", fontsize=10)
    ax[2].set_ylabel("Cul PDE loss of pred", fontsize=10)
    # ax[3].set_ylabel("phys loss of pred", fontsize=10)
    ax[fig_num - 1].set_xlabel("t", fontsize=10)

    for k in range(len(log_list)):
        data_list = torch.load(f'logs/data/phase1_test_{log_list[k]}_{ex_name}')
        _, Lpde_obs, Lpde_pred, _, Lpde_pred_cul, _, _, _, _ = calMean(data_list)
        _, Lpde_obs_v, Lpde_pred_v, _, Lpde_pred_cul_v, _, _, _, _ = calVar(data_list)
    ...
    plt.savefig(f'logs/pics/phase1_Lpde_{fig_name}_{ex_name}.jpg')
...
def coef_plot(t_nn, scale_k, data, fig_name):
    ...
    ax[0].set_title("Test Obs", fontsize=15)
    ax[0].set_ylabel(r'$C_D$', fontsize=10)
    ax[1].set_ylabel(r'$C_L$', fontsize=10)
    ax[fig_num-1].set_xlabel("t", fontsize=10)
