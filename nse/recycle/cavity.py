...
def main():
    ...
    xs = np.linspace(0, 1, N_POINTS_P_AXIS)
    ys = np.linspace(0, 1, N_POINTS_P_AXIS)
    ...
    out = np.zeros((N_TIME_STEPS, N_POINTS_P_AXIS, N_POINTS_P_AXIS, 3))
    for t in tqdm(range(N_TIME_STEPS)):
        ...
        for i in range(N_POINTS_P_AXIS):
            for j in range(N_POINTS_P_AXIS):
                xy = grids[i, j]
                out[t, i, j] = u_next(xy)
