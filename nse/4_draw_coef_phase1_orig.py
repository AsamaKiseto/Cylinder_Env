...
if __name__ == '__main__':
    ...
    t = np.arange(nt) * 0.01 * nt + 0.01 * nt
    ...
    obs_nn[:t_start] = data[k, 1:t_start+1]
    obs_sps = data[k][1:]
    ...
    obs_var = (obs_var.reshape(nt, -1)**2).mean(1)
    ax3.plot(t, obs_var)
    ax3.grid(True, lw=0.4, ls="--", c=".50")
    ax3.set_ylabel(r"$state$", fontsize=15)
    ax3.set_xlim(0, 4)

    plt.savefig(f'logs/coef_phase1_#{k}_t_start_{t_start}.jpg')