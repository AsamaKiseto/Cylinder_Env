from CFD_GPU import MyGeometry, MyFunctionSpace, MySolver   
import copy
from fenics import *
import numpy as np
...
class Cylinder_Rotation_Sim:
    def __init__(self,  params):
        ...
        self.init_state_0_vector.vector()[:] = solver.sol_.vector()
        self.solver.init_solve() 
        self.init_state_1_vector = Function(self.function_space.V)
        self.init_state_1_vector.vector()[:] = solver.sol_.vector()
        # self.init_state_1_vector =   solver.sol.vector()
    
    def set_state_funcval(self, initu, initp):
        assign(self.solver.sol_, [initu, initp])
    
    def set_state_vector(self, init_state_vector):
        self.solver.sol_.vector()[:] = init_state_vector
    ...
    def get_state(self):
        return self.solver.sol_.vector()
    ...
    def get_observation(self, mode):
        if mode == 'vertex':
            vertex_obs = self.solver.sol_.compute_vertex_values()
            ...
            for i in range(self.grids.shape[0]):
                for j in range(self.grids.shape[1]):
                    ...
                    if np.linalg.norm(xy - center) > r:
                        out[i, j] = self.solver.sol_(xy)         # sol & sol_n
                    else:
                        out[i, j] = 0
